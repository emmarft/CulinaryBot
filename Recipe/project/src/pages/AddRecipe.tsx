...
export function AddRecipe() {
  ...
  useEffect(() => {
    // Récupère la session utilisateur à l'initialisation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!user) {
      setMessage('Vous devez être connecté pour ajouter une recette.')
      return
    }
  ...
  if (!user) {
    return (
      <div className="max-w-2xl mx-auto p-6 text-center">
        <p className="mb-4 text-red-600 font-semibold">Vous devez être connecté pour ajouter une recette.</p>
        <button
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white shadow-sm rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {message && (
            <div className={`p-4 rounded-md ${message.includes('succès') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
              }`}>
              {message}
            </div>
          )}
